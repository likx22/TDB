...
RC BplusTreeIndex::insert_entry(const char *record, const RID *rid)
{
  ...
  for (const auto& field_meta : multi_field_metas_) {
    multi_keys.push_back(record + field_meta.offset());
  }

  RC rc = RC::SUCCESS;
  if (index_meta_.is_unique()) {
    std::list<RID> rids;
    rc = index_handler_.get_entry(multi_keys.data(), rids, multi_field_metas_.size());
    if (!rids.empty()) {
  ...
  rc = index_handler_.insert_entry(multi_keys.data(), rid);
  if (rc != RC::SUCCESS) {
