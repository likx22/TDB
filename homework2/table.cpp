...
{
  RC rc = record_handler_->insert_record(record.data(), table_meta_.record_size(), &record.rid());
  if (rc != RC::SUCCESS) {
  ...
  if (rc != RC::SUCCESS) {
    ...
    for (Index *index : indexes_) {
      ...
      if (insert_rc != RC::SUCCESS) {
        LOG_ERROR("Failed to rollback index deletion. table=%s, index=%s, rc=%d:%s",
                  name(), index->index_meta().name(), insert_rc, strrc(insert_rc));
      }
    }
  }
