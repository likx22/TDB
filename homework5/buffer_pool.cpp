...
RC FileBufferPool::evict_page(PageNum page_num, Frame *buf) {
    ...
    if (buf->dirty()) {
        ...
        if (rc != RC::SUCCESS) {
            LOG_ERROR("Failed to flush dirty page {} during eviction", page_num);
            return rc;
        }
    }
...
RC FileBufferPool::evict_all_pages() {
    ...
    RC final_rc = RC::SUCCESS;
    frame_manager_.evict_frames(std::numeric_limits<int>::max(), [this, &final_rc](Frame *frame) -> RC {
        if (frame->dirty()) {
            RC rc = this->flush_page_internal(*frame);
            if (rc != RC::SUCCESS) {
                LOG_ERROR("Failed to flush dirty page {} during eviction", frame->page_num());
                final_rc = rc;
                return rc;
            }
        }
    ...
    // 在所有页面刷新后，执行一次 fsync 以确保所有数据都写入磁盘
    if (final_rc == RC::SUCCESS && fsync(file_desc_) == -1) {
        LOG_ERROR("Failed to fsync file after evicting all pages: {}", strerror(errno));
        final_rc = RC::IOERR_WRITE;
    }
...
RC BufferPoolManager::flush_page(Frame &frame) {
    ...
    auto it_fd = fd_buffer_pools_.find(frame.file_desc());
    if (it_fd == fd_buffer_pools_.end()) {
        LOG_ERROR("FileBufferPool not found for file descriptor: {}", frame.file_desc());
        return RC::FILE_NOT_OPENED;
    }
    
    RC rc = it_fd->second->flush_page(frame);
    if (rc != RC::SUCCESS) {
        LOG_ERROR("Failed to flush page {} for file descriptor {}", frame.page_num(), frame.file_desc());
    }
    return rc;
}
