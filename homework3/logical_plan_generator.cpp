...
RC LogicalPlanGenerator::plan_node(SelectStmt* select_stmt, unique_ptr<LogicalNode>& logical_node) {
    ...
    root = std::unique_ptr<LogicalNode>(new TableGetLogicalNode(default_table, select_stmt->table_alias()[0], fields, true /*readonly*/));
    ...
    for (auto* table : tables) {
        ...
        table_get_nodes.push_back(std::unique_ptr<LogicalNode>(new TableGetLogicalNode(table, table->name(), fields, true /*readonly*/)));
    }
    ...
    // 7. project node
    unique_ptr<LogicalNode> project_logical_node = unique_ptr<LogicalNode>(new ProjectLogicalNode(select_stmt->projects()));
